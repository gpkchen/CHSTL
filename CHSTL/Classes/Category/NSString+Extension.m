...
- (BOOL)isTelNumber{
    ...
    NSString *regex = @"^((13[0-9])|(15[0-9])|(18[0-9])|(17[0-9])|(14[0-9]))\\d{8}$";
    NSPredicate *pred = [NSPredicate predicateWithFormat:@"SELF MATCHES %@", regex];
    BOOL isMatch = [pred evaluateWithObject:self];
    if (!isMatch)
        return NO;
...
- (NSString *)URLEncode{
    NSString *encodeStr = [self stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet URLHostAllowedCharacterSet]];
    encodeStr = [encodeStr stringByReplacingOccurrencesOfString:@"=" withString:@"%3d"];
    encodeStr = [encodeStr stringByReplacingOccurrencesOfString:@"&" withString:@"%26"];
    return encodeStr;
}

-(NSString *)URLDecode{
    NSString *replaceStr = [self stringByReplacingOccurrencesOfString:@"+" withString:@"%20"];
    NSString *decodedString  = (__bridge_transfer NSString *)CFURLCreateStringByReplacingPercentEscapesUsingEncoding(NULL,
                                                                                                                     (__bridge CFStringRef)replaceStr,
                                                                                                                     CFSTR(""),
